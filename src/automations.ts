...
// @ts-ignore
import * as SunCalc from 'suncalc'; // Use https://www.latlong.net/ to get latidute and longitude based on your adress 43.742390 7.424550
...
class AutomationsExtension {
    ...
    constructor(
        ...
        Object.keys(this.eventAutomations).forEach(key => {
            ...
            eventAutomationArray.forEach(eventAutomation => {
                this.log.info(`- key: #${key}# automation: ${this.stringify(eventAutomation)}`);
            });
        });
        ...
        Object.keys(this.timeAutomations).forEach(key => {
            ...
            timeAutomationArray.forEach(timeAutomation => {
                this.log.info(`- key: #${key}# automation: ${this.stringify(timeAutomation)}`);
    ...
    private parseConfig(configAutomations: ConfigAutomations | string) {
        ...
        Object.entries(configAutomations).forEach(([key, configAutomation]) => {
            ...
            if (!platforms.includes(configAutomation.trigger.platform)) {
                this.logger.error(`[Automations] Config validation error for [${key}]: unknown trigger platform '${configAutomation.trigger.platform}'`);
                return;
            }
            // Check actions
            ...
            switch (configAutomation.trigger.platform) {
                ...
                case ConfigPlatform.SUNCALC: {
                    ...
                    this.logger.info(`[Automations] Sunrise at ${times.sunrise.toLocaleTimeString()} sunset at ${times.sunset.toLocaleTimeString()} for latitude:${configAutomation.trigger.latitude} longitude:${configAutomation.trigger.longitude}`);
                ...
                case ConfigPlatform.STATE: {
                    this.logger.info(`[Automations] Registering event automation [${key}]`);
                    if (!configAutomation.trigger.entity) {
                        this.logger.error(`[Automations] Config validation error for [${key}]: trigger entity not specified`);
                        return;
                    }
                    if (!this.zigbee.resolveEntity(configAutomation.trigger.entity)) {
                        this.logger.error(`[Automations] Config validation error for [${key}]: trigger entity #${configAutomation.trigger.entity}# not found`);
                        return;
                    }
                    const entities = toArray(configAutomation.trigger.entity);
                    for (const entityId of entities) {
                        if (!this.eventAutomations[entityId]) {
                            this.eventAutomations[entityId] = [];
                        }
                        this.eventAutomations[entityId].push({ name: key, trigger: configAutomation.trigger, action: actions, condition: conditions });
                    }
                    break;
                }
                default:
    ...
    private startTimeTriggers(key: TimeId, automation: TimeAutomation): void {
        ...
        if (match) {
            ...
            if (timeEvent.getTime() > now.getTime()) {
                this.log.warning(`Set timout at ${timeEvent.toLocaleString()} for [${automation.name}]`);
                this.logger.warning(`[Automations] Set timout at ${timeEvent.toLocaleString()} for [${automation.name}]`);

                const timeout = setTimeout(() => {
                    delete this.timeouts[automation.name];
                    this.log.warning(`timout for [${automation.name}]`);
                    this.logger.warning(`[Automations] timout for [${automation.name}]`);
                ...
                this.log.warning(`timout at ${timeEvent.toLocaleString()} is passed for [${automation.name}]`);
                this.logger.warning(`[Automations] timout at ${timeEvent.toLocaleString()} is passed for [${automation.name}]`);
            }
        } else {
            this.log.warning(`Timout config error at ${key} for [${automation.name}]`);
            this.logger.warning(`[Automations] Timout config error at ${key} for [${automation.name}]`);
        }
    }
    ...
    async stop() {
        this.logger.warning(`[Automations] Extension unloading`);
        for (const key of Object.keys(this.timeouts)) {
            this.logger.warning(`[Automations] Clearing timeout ${key}`);
            clearTimeout(this.timeouts[key]);
            delete this.timeouts[key];
        }
        this.logger.warning(`[Automations] Removing listeners`);
        this.eventBus.removeListeners(this);
        this.logger.warning(`[Automations] Extension unloaded`);
    }
